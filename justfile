...
test: hex0_bin
  bash -euxo pipefail -c '\
    ...
    {{build_dir}}/hex0.bin > {{build_dir}}/full_chain.out 2>/dev/null || status=$?; \
    if [[ "$status" -eq 0 || "$status" -eq 124 ]] && cmp -s {{build_dir}}/full_chain.out <(printf "test"); then \
      printf "PASS\n"; \
    else \
      printf "FAIL\n"; \
      exit 1; \
