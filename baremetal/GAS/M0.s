...
# Building:
# riscv64-none-elf-as -march=rv64i -mabi=lp64 M0_riscv64_reference.s -o M0.o
# riscv64-none-elf-ld --no-relax -Ttext=0x80000000 M0.o -o M0
...
input_done:
    mv s3, s2                         # s3 = end of input buffer
    mv s2, t1                         # s2 = start of input buffer
...
# read_uart function
# Returns character in a0, or 4 (Ctrl-D) on EOF
read_uart:
...
# Using input buffer s2/s3 and Head s4
...
Reverse_List_Loop:
    ...
    ld a2, (a1)                       # NEXT = HEAD->NEXT
    sd a0, (a1)                       # HEAD->NEXT = ROOT
    mv a0, a1                         # ROOT = HEAD
    mv a1, a2                         # HEAD = NEXT
...
Identify_Macros_Loop:
    ...
    ld a0, (a2)                       # I->NEXT
    ld a0, (a0)                       # I->NEXT->NEXT
    ld a0, 16(a0)                     # I->NEXT->NEXT->TEXT
    sd a0, 24(a2)                     # I->EXPRESSION = I->NEXT->NEXT->TEXT

    ld a0, (a2)                       # I->NEXT
    ld a0, (a0)                       # I->NEXT->NEXT
    ld a0, (a0)                       # I->NEXT->NEXT->NEXT
    sd a0, (a2)                       # I->NEXT = I->NEXT->NEXT->NEXT
...
Print_Hex_Next:
    ld a1, (a1)                       # Iterate to next Token
...
# Malloc function
# Simple bump allocator using heap at s1
# Requires a0 to have the number of desired bytes
# Returns pointer in a0
malloc:
    mv t0, s1                         # Store the current pointer
    add s1, s1, a0                    # Bump the pointer by requested bytes
    mv a0, t0                         # Return the old pointer
    ret                               # return
...
fputc:
    ...
    li a1, 0x10000005                 # UART_LSR
fputc_wait:
    lb t0, 0(a1)                      # Read LSR
    andi t0, t0, 0x20                 # Check bit 5 (THR Empty)
    beq t0, zero, fputc_wait          # If TX not ready, wait

    li a1, 0x10000000                 # UART_BASE
